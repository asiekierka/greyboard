...
Canvas.prototype.drawImageURL = function(url,x,y,callback) {
  ...
  var ca = this.canvas;
  img.src = url;
  img.onload = (function(){ ca.drawImage(img,0,0); if(typeof callback != "undefined") callback(); });
...
PointBrush.prototype.draw = function(nx,ny,origcanv,precalc) {
  ...
  var tmp = this.pickXY(nx,ny,typeof precalc !== 'undefined' ? precalc : false);
...
function recursivePointDraw(brush, points, canvas, i, x, y) {
  if(i>=points.length || !points || !brush) return;
  var px = x+points[i].x;
  var py = y+points[i].y;
