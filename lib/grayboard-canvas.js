...
function drawCommand(cmd,canv,config) {
  if(!(_.isObject(canv))) return false;
  try {
    ...
    if(cmd.name == "points") {
      ...
      if(cmdPointBrush.size > config.room.maxBrushSize) return true;
  ...
  catch(e) { console.log("DrawCommand error: " + e.message()); return false; }
  return false;
}
